...
void fifo(char* tracefile, int num_frames, char* execution) {
    vector <memorysystem> fifo_pages; // Vector to store pages in FIFO order
    ...
    while (fscanf(read_file, "%x %c", &single_read.address, &single_read.function) != EOF) {
        ...
        for (i = 0; i < fifo_pages.size(); i++) {
            if (fifo_pages[i].address != single_read.address)
                continue;
            else {
                switch (fifo_pages[i].function) {
                    case 'R':
                        switch (single_read.function) {
                            case 'W':
                                fifo_pages[i].function = 'W'; // If the functions differ, update the function of the existing trace to 'W'
                                break;
                        }
                        break;
                }
        ...
        if (!trace_exists) {
            if (fifo_pages.size() >= num_frames) {
                switch (fifo_pages.front().function) {
                    case 'W':
                        write_count++; // If the ejected/replaced page had function 'W', increment the write count
                        break;
                }
                fifo_pages.erase(fifo_pages.begin()); // Erase the first element of the vector
                fifo_pages.push_back(single_read); // Push the new element at the end of the vector
